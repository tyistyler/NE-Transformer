...
class Trainer(object):
    ...
    def __init__(self, train_data, model, optimizer=None, loss=None,
        ...
        # self.zen_train_dataset = zen_train_dataset
        # self.zen_dev_dataset = zen_dev_dataset
        self.ngram_train_dataset = ngram_train_dataset
        self.ngram_dev_dataset = ngram_dev_dataset
        ...
        if self.dev_data is not None:
            self.tester = Tester(model=self.model,
                                 ...
                                 ngram_test_dataset=self.ngram_dev_dataset
                                 )
    ...
    def get_ngram_data(self, indices, max_seq_len):
        batch_ngram_ids, batch_ngram_positions, batch_attention_mask, batch_ngram_attention_mask = [], [], [], []
        for index in indices:
            input_ids, input_mask, _, _, ngram_ids, ngram_positions, _, _, ngram_masks, _, _ = self.ngram_train_dataset[index]
            batch_ngram_ids.append(ngram_ids.unsqueeze(0))
            batch_ngram_positions.append(ngram_positions.unsqueeze(0))
            batch_ngram_attention_mask.append(ngram_masks.unsqueeze(0))
        batch_ngram_ids = torch.cat(batch_ngram_ids, dim=0).to(self._model_device)
        ...
        # print('batch_ngram_ids',batch_ngram_ids.size())
        # print('batch_ngram_positions',batch_ngram_positions.size())
        # print('batch_ngram_attention_mask',batch_ngram_attention_mask.size())

        # batch_ngram_ids = batch_ngram_ids[:,1:1+max_seq_len]
        # batch_ngram_positions = batch_ngram_positions[:, 1:1 + max_seq_len]
        # batch_ngram_attention_mask = batch_ngram_attention_mask[:, 1:1 + max_seq_len]

        return batch_ngram_ids, batch_ngram_positions, batch_ngram_attention_mask
        ...
        with inner_tqdm(total=self.n_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True) as pbar:
            ...
            for epoch in range(1, self.n_epochs + 1):
                ...
                for indices, batch_x, batch_y in data_iterator:
                    ...
                    if self.use_ngram:
                        input_ngram_ids, ngram_position_matrix, ngram_attention_mask = self.get_ngram_data(indices, max_seq_len=torch.max(batch_x.get("seq_len")).item())
                    batch_x["input_ngram_ids"] = input_ngram_ids
                    batch_x["ngram_position_matrix"] = ngram_position_matrix
                    batch_x["ngram_attention_mask"] = ngram_attention_mask

                    # print(features)
                    # print(features.shape)
                    # batch_x: {"target": tensor, "seq_len": tensor, "chars": tensor}
                    # print("~~~~~~~~\n", indices)
                    # print("========\n", batch_x)
                    # print("--------\n", batch_y, "\n========")
                    # exit()
                    ...
                    # 暂时修改下这里，修改的地方为把下一行注释了
                    self._grad_backward(loss)

                    # 下面几行为新增的东西
                    # loss.backward()
                    # self.fgm.attack()
                    # prediction_fgm = self._data_forward(self.model, batch_x)
                    # loss_fgm = self._compute_loss(prediction_fgm, batch_y).mean()
                    # loss_fgm = loss_fgm / self.update_every
                    # loss_fgm.backward()
                    # self.fgm.restore()
                    # self.optimizer.step()
    ...
    def _data_forward(self, network, batch_x):
        ...
        y = network(
            ...
            batch_x.get("features"),
            batch_x.get("zen_input"),
            batch_x.get("words", None),
            batch_x.get("input_ngram_ids"),
            batch_x.get("ngram_position_matrix"),
            batch_x.get("ngram_attention_mask")
        )
