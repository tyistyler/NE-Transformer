...
class Tester(object):
    ...
    def __init__(self, data, model, metrics, batch_size=16, num_workers=0, device=None, verbose=1, use_tqdm=True,
                 ...
                 zen_model=None, zen_test_dataset=None,
        ...
        self.zen_test_dataset = zen_test_dataset
        self.ngram_test_dataset = ngram_test_dataset
    ...
    def get_features(self, indices, seq_len):
        ...
        for index in indices:
            feature_data = self.test_feature_data[index]
    ...
    def get_zen_data(self, indices, max_seq_len):
        batch_input_ids, batch_ngram_ids, batch_ngram_positions, batch_attention_mask, batch_ngram_attention_mask = \
            [], [], [], [], []
        for index in indices:
            input_ids, input_mask, _, _, ngram_ids, ngram_positions, _, _, ngram_masks, _, _ = self.zen_test_dataset[index]
    ...
    def get_ngram_data(self, indices, max_seq_len):
        ...
        for index in indices:
            input_ids, input_mask, _, _, ngram_ids, ngram_positions, _, _, ngram_masks, _, _ = self.ngram_test_dataset[index]
            batch_ngram_ids.append(ngram_ids.unsqueeze(0))
            batch_ngram_positions.append(ngram_positions.unsqueeze(0))
            batch_ngram_attention_mask.append(ngram_masks.unsqueeze(0))
        batch_ngram_ids = torch.cat(batch_ngram_ids, dim=0).to(self._model_device)
