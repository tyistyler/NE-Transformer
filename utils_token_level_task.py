...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, ngram_dict):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        ngram_matches = []
        #  Filter the ngram segment from 2 to 5 to check whether there is a ngram
        for p in range(2, 6):
            for q in range(0, len(tokens) - p + 1):
                ...
                if character_segment in ngram_dict.ngram_to_id_dict:
                    # print("character_segment:    ",character_segment)
                    ngram_index = ngram_dict.ngram_to_id_dict[character_segment]
                    ngram_matches.append([ngram_index, q, p, character_segment])
                    # print("res: ",[ngram_index, q, p, character_segment])
        ...
        ngram_positions = [ngram[1] for ngram in ngram_matches] # ngram首字符在句子中的位置
        ...
        # for i, x in enumerate(ngram_positions_matrix):
        #     if i < 13:
        #         print("matrix:  ", x)
